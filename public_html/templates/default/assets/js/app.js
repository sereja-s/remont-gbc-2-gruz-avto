(() => {
    ...
    (function(global, factory) {
        ...
        class PatternCursor {
            ...
            _pushLeft(fn) {
                this.pushState();
                for (this.bindBlock(); 0 <= this.index; --this.index, this.offset = this.block?.displayValue.length || 0) if (fn()) return this.ok = true;
        ...
        class RepeatBlock extends MaskedPattern {
            ...
            _update(opts) {
                ...
                this.repeat = repeat ?? block.repeat ?? this.repeat ?? 1 / 0;
                super._update({
            ...
            _appendCharRaw(ch, flags) {
                if (flags === void 0) flags = {};
                const details = new ChangeDetails;
                for (let block, allocated, bi = this._mapPosToBlock(this.displayValue.length)?.index ?? Math.max(this._blocks.length - 1, 0); block = this._blocks[bi] ?? (allocated = !allocated && this._allocateBlock(bi)); ++bi) {
                    const blockDetails = block._appendChar(ch, {
                        ...flags,
                        _beforeTailState: flags._beforeTailState?._blocks?.[bi]
                    });
    ...
    let formValidate = {
        ...
        validateInput(formRequiredItem) {
            ...
            } else if (formRequiredItem.dataset.required === "imask") {
                const phoneMask = new IMask(formRequiredItem, {
    ...
    function initSliders() {
        if (document.querySelector(".gallery__slider1")) new swiper_core_Swiper(".gallery__slider1", {
...
***************************************************************************** */
    ...
    const phoneForms = document.querySelectorAll("[data-imask]");
    phoneForms.forEach((phoneForm => {
        let phoneInput = phoneForm.querySelector("[data-required='imask']");
        let btn = phoneForm.querySelector(".phone-mask-button");
        ...
        function phoneInputHandler() {
            if (phoneMask.masked.isComplete) {
                btn.classList.add("mask--active");
                phoneInput.classList.add("mask--active");
            } else {
                btn.classList.remove("mask--active");
                phoneInput.classList.remove("mask--active");
            }
        }
    }));
    window["FLS"] = true;
