...
function createSitemap() {
	...
	Ajax({ data: { ajax: 'sitemap', links_counter: links_counter } })
		.then((res) => {
			...
			alert("Карта сайта создана успешно")
		})
		.catch((res) => {
			//console.log('ошибка Ajax - ' + res);
			alert("Карта сайта не создана")
			createSitemap();
		});
}
...
function showHideMenuSearch() {
	...
	// Для кнопки меню в админке (id = "hideButton" - для div внутри которого расположен другой div с изобаржением меню):
	document.querySelector('#hideButton').addEventListener('click', () => {
		...
		// убирает класс поданный на вход: vg-hide при каждом клике) При этом при добавлении vg-hide, меню админки скрывается
		document.querySelector('.vg-carcass').classList.toggle('vg-hide');
	});


	// Для кнопки поиска в админке (id = "searchButton" - для div внутри которого расположен другой div с изобаржением поиска):
...
// Выпуск №99 javascript: замыкания, события наведения мыши (РАБОТА С ПОДСКАЗКАМИ ПРИ ВВОДЕ СТРОКИ В ПОЛЕ ПОИСКА)
